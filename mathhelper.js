...
const unflatten = (a, ...newSize) => {
    ...
    return [
        unflatten(a.slice(0, a.length / newSize[0]), ...newSize.slice(1)),
        ...unflatten(
            a.slice(a.length / newSize[0]),
            ...newSize.map((x, i) => (i === 0 ? x - 1 : x))
        ),
    ];
};
...
const flatten = (a) => {
    ...
    while (true) {
        ...
        const currentElement = currentList[i];
        if (currentElement.length === undefined) {
            result.push(currentElement);
            i += 1;
            continue;
        }
        stack.push([currentList, i + 1]);
        [currentList, i] = [currentElement, 0];
    }
};
...
const dotproduct = (a, b) => {
    ...
    if (a.length == b.length)
        return matadd(matmult(a[0], b[0]), dotproduct(a.slice(1), b.slice(1)));
    throw "Mismatched size";
};
...
const crossProduct = (u, v) => {
    if (u.length !== 3 || v.length !== 3)
        throw "Must use 2 3-dimensional vectors!";
    return [
        u[1] * v[2] - u[2] * v[1],
        u[2] * v[0] - u[0] * v[2],
        u[0] * v[1] - u[1] * v[0],
...
const matMult = (A, B) => {
    if (!B[0].length) return A.map((row) => dot(row, B));
    if (A[0].length !== B.length)
        throw `INVALID [${A.length}x${A[0].length}]*[${B.length}x${B[0].length}]`;
