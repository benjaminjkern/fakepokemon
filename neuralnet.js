...
const newKernelLayer = (inputChannels, outputChannels, kernelSpecs, sigma, randomRange = 1) => {
    ...
    return {
        ...
        getAdjustments(delta) {
            this.dbias = zerosTensor([outputChannels]);

            const newDelta = delta.map(() => zerosTensor([...processedKernelSpecs.inputSize, inputChannels]));
            for (const [a, A] of delta.entries()) {
                const adjust = elementWise([...processedKernelSpecs.outputSize, outputChannels], ([ox, oy, i]) =>
                    A.get([ox, oy, i]) *
                    this.sigma(this.lastOutput[a].get([ox, oy, i]), true));
